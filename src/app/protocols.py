...
class Tool(Protocol):
    ...
    async def execute(self, args: JsonObject) -> JsonObject:
        ...
        Returns:
            A ``JsonObject`` representing the tool result.  The model adapter
            serialises this to the wire format (e.g. JSON string) before
            forwarding it to the backend.  Values must never contain raw
            credentials.
        """
        ...
